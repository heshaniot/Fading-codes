...
function sim_PCTC_QAM_par_save1by1(RunID,TxRx,PCTC,label) 
...
% %   FER = zeros(1,length(TxRx.Sim.EbNo_dB_list));
  % -- initialize
  BER_local = zeros(1,TxRx.Sim.nr_of_channels); 
  FER_local = zeros(1,TxRx.Sim.nr_of_channels);
...
%     for k=1:length(TxRx.Sim.EbNo_dB_list)
      ...
      y = modulatedBitStream + noise*sqrt(sigma2);
