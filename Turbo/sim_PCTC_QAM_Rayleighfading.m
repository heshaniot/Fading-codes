...
function sim_PCTC_QAMfading_save1by1(RunID,TxRx,PCTC,label) 
  ...
  %coderate
  R = PCTC.InformationBits/PCTC.TotalCodedBits;
%   sigma2 = bitsPerSymbol/(2*PCTC.Rate)*10.^(-TxRx.Sim.EbNo_dB_list/10);
 
  EbNo_dB_list=TxRx.Sim.EbNo_dB_list;

  %Energy per Symbol
  Es = bitsPerSymbol*R*No*10.^(EbNo_dB_list/10);
  
  str = sprintf('Turbo_R%d_QAM_N%d_K%d_Iter%d_%.2fdB_1e%d',label.R,PCTC.TotalCodedBits,PCTC.InformationBits,TxRx.Decoder.Iterations,TxRx.Sim.EbNo_dB_list,log10(TxRx.Sim.nr_of_channels));
  ...
  for trial=1:TxRx.Sim.nr_of_channels
    ...
    %normalized modulated simbols (variance =1)
    modulatedBitStream =  (1/sqrt(2))* (  reshapedBitStream(1,:)+ 1i*reshapedBitStream(2,:));
     
    % -- prepare Gaussian Noise    
    noise = randn(1,length(reshapedBitStream))+ 1i*randn(1,length(reshapedBitStream)); 
    
    % Rayleigh fading channel
    H = sqrt(1/2)*( randn(1,length(reshapedBitStream))+ 1i*randn(1,length(reshapedBitStream)) ); 

    
    for k=1:length(TxRx.Sim.EbNo_dB_list)
        
      modulatedBitStream = sqrt(Es(k)).*modulatedBitStream;

      % -- simulate AWGN channel
      y = H.*modulatedBitStream./(H.*transpose(H'))+ noise*sqrt(sigma2);
%         y = H.*modulatedBitStream+ noise*sqrt(sigma2(k));
...
%       y = H.*y; 
      ...
      initialLRs(1:2:end) =2*sqrt(Es(k))* real(y.*transpose(H'))/sigma2; 
      initialLRs(2:2:end) = 2*sqrt(Es(k))* imag(y.*transpose(H'))/sigma2;
%       initialLRs(1:2:end) = (2/sigma2(k)) * real(y.*H); 
%       initialLRs(2:2:end) = (2/sigma2(k)) * imag(y.*H);
