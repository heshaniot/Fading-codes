...
%     H = sqrt(1/2)*( ones(1,length(reshapedBitStream))+ 1i*ones(1,length(reshapedBitStream)) ); 
%     H = randn(1,length(reshapedBitStream));
    for k=1:length(TxRx.Sim.EbNo_dB_list)
        
      % -- simulate AWGN channel
%       y = H.*modulatedBitStream/(H.*transpose(H'))+ noise*sqrt(sigma2(k));
...
%       y = H.*y; 
      ...
      initialLRs = zeros(1,length(CodedBitStream));
          initialLRs(1:2:end) = (2/sigma2(k)) * real(y.*transpose(H')); 
      initialLRs(2:2:end) = (2/sigma2(k)) * imag(y.*transpose(H'));
