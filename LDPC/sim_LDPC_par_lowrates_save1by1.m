...
function sim_LDPC_par_lowrates_save1by1(RunID,TxRx,LDPC,label) 
  ...
  for k=1:EbNo_length
        ...
        Ec = (K/N)*No*10^(EbNodB_range(k)/10); %Energy per coded bit
        ...
        parfor trial=1:MCsize
            ...
            % -- AWGN channel
            channelNoise = sqrt(No/2)*randn(1,length(x)/bitsPerSymbol)+1i*sqrt(No/2)*randn(1,length(x)/bitsPerSymbol);
...
%             initialLRs(2:2:end) = (2/sigma2) * imag(y); 
            initialLRs(1:2:end) = (2*sqrt(Ec)/No) * real(y); 
            initialLRs(2:2:end) = (2*sqrt(Ec)/No) * imag(y);
            LLR_A2= initialLRs;
...
%         end
        fprintf('\n %d dB (timetaken %.2f seconds)',EbNodB_range(k),toc(titer))
          ...
          Results.TxRx = TxRx;
          Results.EbNo = EbNodB_range(k);
          Results.LDPC = LDPC;
          Results.BLER = BLER(k);
          Results.BER = BER(k);

          Results.FileName = sprintf([label.folder '/LDPC_N%d_K%d_R%d_%s_%s_Iter%d_1e%d_EbNo%.2f_point%d.mat'],N,K,label.rate ,modulationMethod,TxRx.Decoder.LDPC.Type,TxRx.Decoder.LDPC.Iterations,log10(MCsize),EbNodB_range(k),label.number);
...
% savefig(filename)
  ...
  Results.FileName = sprintf('%s/LDPC_N%d_K%d_%s_%s_Iter%d_1e%d_.mat',label.folder,N,K,modulationMethod,TxRx.Decoder.LDPC.Type,TxRx.Decoder.LDPC.Iterations,log10(MCsize));

%   Results.FileName = sprintf('%s/LDPC_N%d_K%d_%s_%s_Iter%d_1e%d_EbNo%.2f.mat',label.folder,N,K,modulationMethod,TxRx.Decoder.LDPC.Type,TxRx.Decoder.LDPC.Iterations,log10(MCsize),EbNodB_range);
  save(Results.FileName,'Results');
