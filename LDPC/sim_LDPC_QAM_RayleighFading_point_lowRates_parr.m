...
function sim_LDPC_QAM_RayleighFading_point_lowRates_parr(RunID,TxRx,LDPC,label) 
...
%     BER_local  = zeros(MCsize,1);
  ...
  parfor trial=1:MCsize 
        ...
        H = sqrt(1/2).*(randn(1,length(x)/bitsPerSymbol)+ 1i*randn(1,length(x)/bitsPerSymbol)) ; 
      
        s_mod = sqrt(Es).*s_mod;

        y =H.* s_mod./(H.*transpose(H')) + channelNoise;

        initialLRs = zeros(1,length(x));
%             initialLRs(1:2:end) = (2*sqrt(Es)/No) * real(y); 
%             initialLRs(2:2:end) = (2*sqrt(Es)/No) * imag(y);
        initialLRs(1:2:end) =2*sqrt(Es)* real(y.*transpose(H'))/sigma2; 
        initialLRs(2:2:end) = 2*sqrt(Es)* imag(y.*transpose(H'))/sigma2;
...
%         end    
  ...
  SaveResults(TxRx,LDPC,MCsize,EbNodB_range,BER,BLER,label.folder,str_filename );
...
function SaveResults(TxRx,LDPC,MCsize,EbNodB_range,BER,BLER,folder,str)
  % -- save results to disk
  Results.TxRx = TxRx;
  Results.EbNo = EbNodB_range;
  Results.LDPC = LDPC;
  Results.BLER = sum(BLER)/MCsize;
  Results.BER = sum(BER)/(LDPC.inf_bits.*MCsize);
